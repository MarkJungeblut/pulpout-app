...
Future<WorkoutSchedule> postWorkoutSchedule(WorkoutSchedule workoutSchedule) async {
  final response =
      await post(Uri.parse('http://localhost:8080/workout/schedule'), body: jsonEncode(workoutSchedule.toJson()));
...
Future<List<WorkoutSchedule>> getWorkoutSchedules() async {
  final response = await get(Uri.parse('http://localhost:8080/workout/schedule'));

  if (response.statusCode == 200) {
    Iterable workoutSchedules = jsonDecode(response.body);
    return List<WorkoutSchedule>.from(workoutSchedules.map((exercise) => WorkoutSchedule.fromJson(exercise)));
  }
